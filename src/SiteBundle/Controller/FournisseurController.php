...
class FournisseurController extends Controller
{
    ...
    public function importProductsAction(Request $request)
    {
        ...
        for ($i = 1; $i < count($data); $i++){
            ...
            $descr = empty($ligne[28]) ? " ": iconv("UTF-8", "WINDOWS-1252", $ligne[28]);
            ...
            $refFour = empty($ligne[24]) ? " ": iconv("UTF-8", "WINDOWS-1252", $ligne[24]);
            $refPart = empty($ligne[25]) ? " ": iconv("UTF-8", "WINDOWS-1252", $ligne[25]);
            $ean = empty($ligne[26]) ? " ": iconv("UTF-8", "WINDOWS-1252", $ligne[26]);
            $nomProduit = empty($ligne[27]) ? " ": iconv("UTF-8", "WINDOWS-1252", $ligne[27]);
            $descrLong = empty($ligne[29]) ? " ": iconv("UTF-8", "WINDOWS-1252", $ligne[29]);
